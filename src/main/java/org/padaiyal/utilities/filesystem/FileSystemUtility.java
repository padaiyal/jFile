...
public class FileSystemUtility {
  ...
  public static void setPermissions(
    ...
    if (setRecursively) {
      Stream<Path> pathsFound;
      try {
        pathsFound = Files.walk(path);
  ...
  public static void deleteRecursively(Path path) throws IOException {
    ...
    if (Files.isDirectory(path)) {
      List<Path> dirTreeList = Files.list(path).collect(Collectors.toList());
  ...
  public static void registerWatchServiceForNotRegisteredDirectoryPathAfterFileVisitor(
      Path currentPath, int maxDepth, WatchEvent.Kind<?>[] eventTypesToWatch,
